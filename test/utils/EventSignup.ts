...
import { getEventFactoryContract, getSigner } from "../Context";
...
export class EventContract {
  private static EventFactoryContract: ethers.Contract;
  ...
  public static async initialize(): Promise<void> {
    EventContract.EventFactoryContract = await getEventFactoryContract();
    EventContract.signer = await getSigner();
  }

  static async getContract(addr: string): Promise<EventContract> {
    try {
      const contract = new ethers.Contract(addr, contractABI.abi, EventContract.signer);
  ...
  static async getActiveContracts(): Promise<EventContract[]> {
    try {
      const eventAddresses: string[] = await EventContract.EventFactoryContract.getActiveEvents();
  ...
  static async createContract(input: CreateEventInput, signer: ethers.ContractRunner): Promise<EventContract> {
    // 1. Send transaction to create the event
    const tx = await EventContract.EventFactoryContract.createEvent(
      input.name,
      input.description,
      input.maxCapacity,
      input.signupStartTime.getTime() / 1000,
      input.signupEndTime.getTime() / 1000,
      input.eventStartTime.getTime() / 1000,
      input.eventEndTime.getTime() / 1000,
      input.rewardCost
    );
    ...
    const eventLog = receipt?.logs.find(log => {
      try {
        return this.EventFactoryContract.interface.parseLog(log) !== null;
      } catch {
        return false;
      }
    }) as EventLog;
    ...
    const eventDetails: EventDetails = { ...input, attendeeCount: 0 }
  ...
  public async isAttendee(): Promise<boolean> {
    try {
      const signedUp = await this.contract.isAttendee();
      console.log(`Checked signup is ${signedUp} for address ${await EventContract.signer.getAddress()} on event contract: ${this.contractAddress}`);
      return signedUp;
    } catch (err) {
