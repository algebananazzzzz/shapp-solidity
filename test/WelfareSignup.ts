...
describe("WelfareSignup Contract", function () {
  ...
  async function deployWelfareSignupFixture() {
    ...
    await token.connect(addr1).increaseAllowance(welfareSignup.getAddress(), redemptionCost)
    await token.connect(addr2).increaseAllowance(welfareSignup.getAddress(), redemptionCost)
    await token.connect(addr3).increaseAllowance(welfareSignup.getAddress(), redemptionCost)
    await token.connect(addr4).increaseAllowance(welfareSignup.getAddress(), redemptionCost)

    return welfareSignup;
  }

  it("should create the welfare correctly", async function () {
    welfareSignup = await loadFixture(deployWelfareSignupFixture);
  ...
  it("should not allow non-creator to deactivate the welfare", async function () {
    ...
    await expect(welfareSignup.connect(addr1).deactivate()).to.be.revertedWith("Only the creator can perform this action");
  });
});
